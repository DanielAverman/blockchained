...
public class Transaction {
    ...
    public void generateSignature(PrivateKey privateKey) {
        ...
        signature = StringUtils.applyECDSASignature(privateKey, data);
    }
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput input : inputs) {
            input.UTXO = BlockChainedSystem.UTXOs.get(input.transactionOutputId);
        }

        if (getInputsValue() < BlockChainedSystem.minimumTransaction) {
        ...
        for(TransactionOutput output : outputs) {
            BlockChainedSystem.UTXOs.put(output.id, output);
        }

        for(TransactionInput input : inputs) {
            if (input.UTXO == null) continue;
            BlockChainedSystem.UTXOs.remove(input.UTXO.id);
        }
